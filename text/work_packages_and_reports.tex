...
Our basic erasure-coding chunk size is $\mathsf{W}_E = 684$ octets, derived from the fact we wish to be able to reconstruct even should almost two-thirds of our 1023 participants be malicious or incapacitated, the 16-bit Galois field on which the erasure-code is based and the desire to efficiently support encoding data of close to, but no less than, 4\textsc{kb}.
...
\begin{equation}\label{eq:segment}
  \segment \equiv \blob_{\mathsf{W}_G}
\end{equation}
...
\begin{equation}
  \itemtodigest\colon\abracegroup{
    ...
    \end{aligned}
      ...
      &\wl¬service,\,
      \wl¬codehash,\,
      \is{\wl¬payloadhash}{\blake(\¬payload)},\,
      \is{\wl¬gaslimit}{\wi¬accgaslimit},\,
      \wl¬result,\,
      \wl¬gasused,\\
      &\is{\wl¬importcount}{\len{\wi¬importsegments}},\,
      \wl¬exportcount,\,
      \is{\wl¬xtcount}{\len{\wi¬extrinsics}},\,
      \is{\wl¬xtsize}{\!\!\!\!\sum_{\tup{h, z} \in \wi¬extrinsics}\!\!\!\!z}
    \end{aligned}}\!\!\!\!
  }
\end{equation}
...
\begin{equation}
  \forall \wpX \in \workpackage: \abracegroup[\,]{
    ...
    \se(\var{\wpX_\wp¬metadata}, \wpX_\wp¬authcode) &\equiv \histlookup(\accounts[\wpX_\wp¬authcodehost], (\wpX_\wp¬context)_t, \wpX_\wp¬authcodehash) \\
...
Where:
\begin{align*}
  \Fkeys{\wr¬srlookup} \equiv \,&\set{ h \mid \wiX \in \wpX_\mathbf{w}, (h^\boxplus, n) \in \wiX_\mathbf{i} } \ ,\quad\len{\wr¬srlookup} \le 8\\
  ...
  (\wr¬digests, \allexports) = \,&\transpose[
    ...
    j \orderedin \N_{\len{\wpX_\wp¬workitems}}
  ] \\
  \countupexports(\wpX, j) \equiv \,&\begin{cases}
    (\oversize, u, [\segment_0, \segment_0, \dots]_{\dots w_e}) &\when \len{r} + z > \mathsf{W}_R\\
    (\badexports, u, [\segment_0, \segment_0, \dots]_{\dots w_e}) &\otherwhen \len{\mathbf{e}} \ne w_e \\
    (r, u, [\segment_0, \segment_0, \dots]_{\dots w_e}) &\otherwhen r \not\in \blob \\
    ...
    \multicolumn{2}{l}{\also h = \blake(\wpX)\,,\; w = \wpX_\wp¬workitems[j]\,,\; \ell = \sum_{k < j}\wpX_\wp¬workitems[k]_\wi¬exportcount}\\
    \multicolumn{2}{l}{\also z = \len{\mathbf{o}} + \sum_{k < j, (r \in \blob, \dots) = I(p, k)} \len{r}}
  \end{cases}
\end{align*}
...
\begin{equation}
  \forall (h \mapsto e) \in \wr¬srlookup : \exists \wpX, \¬core \in \workpackage, \coreindex : \blake(\wpX) = h \wedge (\computedigest(\wpX, \¬core)_s)_e = e\!\!\!\!
\end{equation}
...
\begin{equation}\label{eq:zeropadding}
  \zeropad_{n \in \N_{1\dots}}\colon\abracegroup{
    \blob &\to \blob_{k \cdot n}\\
